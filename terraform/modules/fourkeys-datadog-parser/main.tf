...
resource "google_cloud_run_service" "datadog_parser" {
  project  = var.project_id
  name     = "fourkeys-datadog-parser"
  location = var.region
...
resource "google_pubsub_topic" "datadog" {
  project = var.project_id
  name    = "datadog"
}
...
resource "google_pubsub_topic_iam_member" "service_account_editor" {
  project = var.project_id
  topic   = google_pubsub_topic.datadog.id
  role    = "roles/editor"
...
resource "google_pubsub_subscription" "datadog" {
  ...
  push_config {
    push_endpoint = google_cloud_run_service.datadog_parser.status[0]["url"]
