...
ARRAY_AGG(DISTINCT root_cause IGNORE NULLS) changes,
FROM
(
SELECT 
source,
CASE WHEN source LIKE "github%" THEN JSON_EXTRACT_SCALAR(metadata, '$.issue.number')
     ...
     WHEN source LIKE "datadog%" THEN JSON_EXTRACT_SCALAR(metadata, '$.incident_id')
     END AS incident_id,
CASE WHEN source LIKE "github%" THEN TIMESTAMP(JSON_EXTRACT_SCALAR(metadata, '$.issue.created_at'))
     ...
     WHEN source LIKE "datadog%" AND JSON_EXTRACT_SCALAR(metadata, '$.status') = "active" THEN time_created
     END AS time_created,
CASE WHEN source LIKE "github%" THEN TIMESTAMP(JSON_EXTRACT_SCALAR(metadata, '$.issue.closed_at'))
     ...
     WHEN source LIKE "pagerduty%" THEN TIMESTAMP(JSON_EXTRACT_SCALAR(metadata, '$.event.occurred_at'))
     WHEN source LIKE "datadog%" AND JSON_EXTRACT_SCALAR(metadata, '$.status') = "resolved" THEN time_created
     END AS time_resolved,
CASE WHEN source LIKE "datadog%" THEN JSON_EXTRACT_SCALAR(metadata, "$.fields.Commit")
     ELSE REGEXP_EXTRACT(metadata, r"root cause: ([[:alnum:]]*)")
     END AS root_cause,
CASE WHEN source LIKE "github%" THEN REGEXP_CONTAINS(JSON_EXTRACT(metadata, '$.issue.labels'), '"name":"Incident"')
     ...
     WHEN source LIKE "datadog%" THEN TRUE # All Datadog events are incident-related
     END AS bug,
